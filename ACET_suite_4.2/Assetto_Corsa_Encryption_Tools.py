...
def nr_align_and_parent():
    ...
    # Before aligning, rotate NR_Alignment_R around the Y axis by 180 degrees. This was guesswork, and is not required anymore with new logic.
...
def import_nr_mesh(directory, files, matrix):
    ...
    bpy.ops.import_mesh.nr(
        ...
        exactProjMat=matrix,
        # Above is defaulted perameters, may need to adjust based on file type
    )
                ...
                for mat_slot in obj.material_slots:
                    if mat_slot.material and mat_slot.material.use_nodes:
                        ...
                        if detail_tex_node.image:
                            ...
                            # If all conditions are met then swap the texture.
                    ...
                    if mat_slot.material and mat_slot.material.use_nodes:
                        ...
                        if not ksmaterial_node:
                            ...
                            for input_name in input_names:
                                node_group.interface.new_socket(input_name, description='', in_out='INPUT', socket_type='NodeSocketBool', parent=None)
                        
                            # Create input sockets for float properties
                            node_group.interface.new_socket("Detail Mult", description='', in_out='INPUT', socket_type='NodeSocketFloat', parent=None)
                            node_group.interface.new_socket("PBR Mult", description='', in_out='INPUT', socket_type='NodeSocketFloat', parent=None)
            ...
            try:
                ...
                if int(_currentTextureCount) == 3:
                    ...
                    if _currentShader == "ksPerPixelAT_NM_emissive":
                        ...
                        links.new(img_tex_3_node.outputs['Color'], principled_bsdf_node.inputs['Emission Color'])
                ...
                if int(_currentTextureCount) == 4:
                    ...
                    if _useDetail == True:
                        ...
                        links.new(separate_color_node.outputs[0], principled_bsdf_node.inputs['Specular IOR Level'])
                ...
                if int(_currentTextureCount) == 5:
                    ...
                    if _useDetail == True:
                        if _currentShader in ("ksDiscBrake", "ksTyres"):
                            ...
                            # Emission
                            links.new(img_tex_5_node.outputs['Color'], principled_bsdf_node.inputs['Emission Color'])
...
class OBJECT_OT_RenameDuplicateObject(bpy.types.Operator):
    ...
    def poll(cls, context):
        # Check if there are selected objects and if all selected objects are meshes
...
# Operator and function to remove the suffixes on materials. 
# Must have at least 1 selected object for the button to be active. 

def drop_material_suffix(context):
    for obj in context.selected_objects:
        if obj.type == 'MESH':
            for mat_slot in obj.material_slots:
                mat = mat_slot.material
                if mat and mat.name.endswith('.001'):
                    original_name = mat.name[:-4]
                    if original_name in bpy.data.materials:
                        # Set to existing material without suffix
                        mat_slot.material = bpy.data.materials[original_name]
                    else:
                        # Rename material by dropping the suffix
                        mat.name = original_name

# The operator to remove material suffixes.

class OBJECT_OT_Drop_Mat_Suffix(bpy.types.Operator):
    """Drop '.001' suffix from material names or replace with existing materials"""
    bl_idname = "acet.drop_mat_suffix"
    bl_label = "Drop Material Suffix"
    
    # The classmethod with poll allows up to perform a check to enable the button
    @classmethod
    def poll(cls, context):
        return context.selected_objects

    def execute(self, context):
        drop_material_suffix(context)
        return {'FINISHED'}
                        ...
                        for index, node in enumerate(nodes):
                            if node.type == 'TEX_IMAGE' and node.image:
                                ...
                                if principled_bsdf_node and node.name == "Image Texture.002":
                                    ...
                                    if principled_bsdf_node and 'Specular IOR Level' in principled_bsdf_node.inputs:
                                        links.new(separate_color_node.outputs[0], principled_bsdf_node.inputs['Specular IOR Level'])
...
class OBJECT_PT_acet_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("acet.drop_mat_suffix", icon='NONE', text="Drop Material Suffix")
...
def register():
    ...
    bpy.utils.register_class(OBJECT_OT_Drop_Mat_Suffix)
...
def unregister():
    ...
    bpy.utils.unregister_class(OBJECT_OT_Drop_Mat_Suffix)
